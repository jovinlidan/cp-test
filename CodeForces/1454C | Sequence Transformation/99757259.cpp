...
int main()
{
    ...
    while(tc--)
    {
        ...
        else
        {
            ...
            if(arr[0] == arr[n-1] && mp[arr[0]] == 2)cout << 1 << '\n';
            else if(mp[arr[0]] == 1)cout << 1 << '\n';
            else if(mp[arr[n-1]] == 1)cout << 1 << '\n';
            else
            {
                if(arr[0] == arr[n-1])
                {
                    mn = min(mn, mp[arr[0]]-1);
                    //debug(mp[3]);
                }

                for(int i = 0; i < n; i++)
                {
                    if(arr[i] == arr[0] || arr[i] == arr[n-1])mn = min(mn, mp[arr[i]]);
                    else mn = min(mn, mp[arr[i]]+1);
                }
